(async function(){
  ...
  function card(s){
    ...
    const desc = s.description_short_en || s.description_short || s.description_small_en || s.description_small || "";
      ...
      <article class="site" itemscope itemtype="https://schema.org/Organization">
        <a class="site-link" href="${url}" itemprop="url" rel="bookmark">
          <div class="site-top">
            ...
            ${s.url ? `<small><a href="${escapeHtml(s.url)}" target="_blank" rel="${externalRel}" style="color: var(--muted); text-decoration: none;">${siteUrl}</a></small>` : `<small>${siteUrl}</small>`}
          </div>
        </a>
        ${s.url ? `
