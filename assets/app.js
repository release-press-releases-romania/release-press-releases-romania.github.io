(async function(){
  ...
  function card(s, index = 0){
    ...
    `;
    const desc = s.description_small_en || s.description_small || s.description_short_en || s.description_short || "";
  ...
  // Load latest 5 Mastodon posts (randomized)
  async function loadLatestMastodon(){
    const mastodonSites = sites.filter(s => s.mastodon && s.mastodon_rss);
    if(mastodonSites.length === 0) {
      q("#latestMastodon").innerHTML = '<div class="notice">No Mastodon feeds available.</div>';
      return;
    }

    const allFeeds = [];
    const maxFeeds = Math.min(20, mastodonSites.length); // Check more feeds to get variety
        ...
        <div class="feed-item">
          ...
          ${Math.random() < 0.2 ? `
            <a href="${link}" target="_blank" rel="nofollow noopener" class="feed-title-image" aria-label="Read Mastodon post: ${title}">
              <img src="/assets/images/logo.svg" alt="${title}" width="20" height="20" style="vertical-align: middle; margin-right: 8px; opacity: 0.7;">
              <span>${title}</span>
            </a>
          ` : `
